...
pub struct Config {
    ...
    label2id: Option<HashMap<String, u8>>
}
...
//https://github.com/huggingface/transformers/blob/96eb06286b63c9c93334d507e632c175d6ba8b28/src/transformers/models/bert/modeling_bert.py#L1845
...
impl BertClassifier {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let dense = linear(config.hidden_size, config.num_labels(),vb.pp("classifier"))?;
        Ok(Self {
            dense,
            span: tracing::span!(tracing::Level::TRACE, "out"),
        })
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
        let classified_states = self.dense.forward(hidden_states)?;
        Ok(classified_states)
    }
}
struct BertDropout {
    dropout: Dropout,
...
pub struct BertModelWithClassifier {
    model: BertModel,
    classifier: BertClassifier, 
    pub device: Device,
    span: tracing::Span,
}
impl BertModelWithClassifier {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let (model, 
            classifier
        ) = match (
            BertModel::load(vb.pp("bert"), &config),
            BertClassifier::load(vb.clone(), config)
        ) {
            (Ok(modek) ,
            Ok(classifier),  
        ) => (modek, 
    ...
    pub fn forward(&self, input_ids: &Tensor, token_type_ids: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
        let sequence_output = self.model.forward(input_ids, token_type_ids)?;
        let classified_output = self.classifier.forward(&sequence_output)?;
        Ok(classified_output)
    }
}